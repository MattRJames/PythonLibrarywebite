...
class TestBook:
    ...
    def test_attribute_setters(self):
        ...
        book.set_image("www.harrypotter.com")
        assert book.get_image() == "www.harrypotter.com"
    ...
    def test_reviews(self):
        ...
        users = [User("Martin", "pw1234"), User("Shkreli", "pw69104")]
        assert book.reviews == []
        assert book.reviews == []
        review1 = Review(book, users[0], "I liked this book", 4)
        review2 = Review(book, users[1], "This book was ok", 2)
        book.add_review(review1)
        book.add_review(review2)
        assert str(book.reviews[0].review_text) == "I liked this book"
        assert book.reviews[0].rating == 4
        assert book.reviews[0].user == users[0]
        assert str(book.reviews[1].review_text) == "This book was ok"
        assert book.reviews[1].rating == 2
        assert book.reviews[1].user == users[1]
...
class TestReview:
    ...
    def test_construction(self):
        ...
        user = User('Yankee', '010001010100010100')
    ...
    def test_attributes_access(self):
        book = Book(2675376, "Harry Potter")
        user = User('Tango', '101101011110000111')
    ...
    def test_score_manipulation(self):
        ...
        review.add_score()
        review.add_score()
        assert review.get_score() == 2
        review.minus_score()
        assert review.get_score() == 1

    def test_invalid_parameters(self):
        book = Book(2675376, "Harry Potter")
        user = User('Uniform', '010001000011010001')
        review_text = "This book was very enjoyable."
    ...
    def test_set_of_reviews(self):
        ...
        user = User('Bravo', '100011001101010100')
...
class TestUser:
    ...
    def test_passwords(self):
        ...
        assert str(user1) == "<User shyamli>"
        assert user1.check_password("pw12345")
        assert str(user2) == "<User martin>"
...
def read_books_and_authors():
    ...
    reader = BooksJSONReader(path_to_books_file, path_to_authors_file, None)
    reader.read_json_files()
    # add authors to books
    for book in reader.dataset_of_books:
        any(map(book.add_author, reader.dataset_of_authors[book]))
...
class TestBooksJSONReader:
    ...
    def test_read_books_from_file_and_check_other_attributes(self, read_books_and_authors):
        ...
        assert dataset_of_books[1].get_tags() == ["bd","to-read","french-author","female-author", "mars-2017","animals","non-fiction","autobiographical", "graphique-bd","graphic-novel","roman-graphique","\u00e9crivaines"]
        assert dataset_of_books[19].get_related_books() == [6801620, 8147688, 6867908, 6772012, 6386382, 11334365, 12712367]
